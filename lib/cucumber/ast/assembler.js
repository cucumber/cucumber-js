var Assembler = function (features, filter) {
  ...
  var self = {
    ...
    finish: function finish() {        
      var scenarios = currentFeature.getScenarios();    
        
      scenarios.syncForEach(function(scenario){
          if(scenario.payload_type === 'scenarioOutline'){
              var subScenarios = scenario.buildScenarios();
              subScenarios.syncForEach(function(sc){                
                  self.insertScenario(sc);
                  sc.getSteps().syncForEach(function(step){
