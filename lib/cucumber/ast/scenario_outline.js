var ScenarioOutline = function (keyword, name, description, uri, line) {
  ...
  self.applyExampleRow = function(example, steps){
    ...
    var new_step = Cucumber.Ast.Step(step.getKeyword(), name, step.getLine());

     if (hasDataTable) {
       rows.forEach(function(row){
         table.attachRow( Cucumber.Ast.DataTable.Row( JSON.parse(row.cells), row.line) );
       });
       new_step.attachDataTable(table);
     }

      if (hasDocString) {
        new_step.attachDocString( Cucumber.Ast.DocString(oldDocString.getContentType(), docString, oldDocString.getLine()));
      }
      return new_step;
    });
  };
