...
//            
//          name = outline.getName().replace(findText, exampleData);         
//            
////          if (hasDataTable) {
...
//      }
//
      var step = Cucumber.Ast.Step(outline.getKeyword(), outline.getName(), outline.getLine());
//
//      if (hasDataTable) {
...
//
  ...
  self.acceptVisitor = function (visitor, callback) {
    ...
    var headers_row = rows.shift().raw();

    rows.syncForEach(function(row, index){
      row.example = {};
      row.id = index.toString();

      for (var i = 0, ii = headers_row.length; i < ii; i++){
        row.example[headers_row[i]] = row.raw()[i];
      }
  ...
  self.visitRowSteps = function (visitor, row, callback) {    
    ...
    self.instructVisitorToVisitBackgroundSteps(visitor, callback);
  };
