var Runtime = function(configuration) {
  ...
  var self = {
    start: function start(callback) {
      ...
      astTreeWalker.walk(function finish(result) {
        if (configuration.getReportFile() != undefined) {
          fs.writeFileSync(configuration.getReportFile(), listeners.getLast().getLogs());
        }
        callback(result);
      });
    },
