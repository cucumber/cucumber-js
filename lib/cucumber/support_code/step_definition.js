...
function StepDefinition (pattern, code, uri, codeType) {
  ...
  var self = {
    ...
    trySourceMap: function(stackFrameMsg) {
      // pull the source file and line out of stack frame -- or give empty strings
      var f = stackFrameMsg.split('(')[1]||'';
      var n = f.lastIndexOf('.');
      n += Math.max(0, f.substr(n).indexOf(':'));
      var targetFile = f.slice(0, n);  // like '~/prj/file.js' or 'C:\\prj\\file.js'
      var position = f.slice(n+1, -1); // like '13:44'

      if (targetFile === '' || position === '') return stackFrameMsg; // can't determine source or position

      try {
        var map = fs.readFileSync(targetFile+'.map');
        map = JSON.parse(map);
        var smc = new sourceMap.SourceMapConsumer(map);
        var realLoc = smc.originalPositionFor({line:+(position.split(':')[0]), column:+(position.split(':')[1])});

        // replace original file location with mapped location
        return stackFrameMsg.replace(map.file, realLoc.source).replace(':'+position, ':'+realLoc.line+':'+realLoc.column);
      } catch (err) {
