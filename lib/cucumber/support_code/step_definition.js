  ...
  var self = {
    ...
    invoke: function invoke(step, world, scenario, stepDomain, defaultTimeout, callback) {
      ...
      if (validCodeLengths.indexOf(code.length) === -1) {
        var error = 'definition has ' + code.length + ' arguments' +
          ', should have ' + validCodeLengths.join(' or ');
        return codeCallback(error);
      }
      ...
      if (callbackInterface && promiseInterface) {
        codeCallback('definition takes a callback and returns a promise');
