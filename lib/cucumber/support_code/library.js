function Library(supportCodeDefinition) {
  ...
  function extendWorld(world) {
    world._timeout = world.defaultTimeout || Library.DEFAULT_TIMEOUT;
    world.timeout = function(newTimeout) {
      if (newTimeout === undefined) {
        return world._timeout;
      } else {
        world._timeout = newTimeout;
      }
    };
    return world;
  }
    ...
    instantiateNewWorld: function instantiateNewWorld(callback) {
      var world = new WorldConstructor(function (explicitWorld) {
        process.nextTick(function () { // release the constructor
          callback(extendWorld(explicitWorld || world));
        });
      });
    }
  };
...
Library.DEFAULT_TIMEOUT = 10 * 1000;
