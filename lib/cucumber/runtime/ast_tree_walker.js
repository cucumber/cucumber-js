...
define([
  ...
  '../runtime',
  './skipped_step_result',
  './undefined_step_result',
  './ast_tree_walker/event'
], function(require, Runtime, SkippedStepResult, UndefinedStepResult, Event) {
var AstTreeWalker = function(features, supportCodeLibrary, listeners) {
  ...
  var self = {
    ...
    skipStep: function skipStep(step, callback) {
      var skippedStepResult = Runtime.SkippedStepResult({step: step});
    ...
    skipUndefinedStep: function skipUndefinedStep(step, callback) {
      var undefinedStepResult = Runtime.UndefinedStepResult({step: step});
