  ...
  var self = {
    ...
    argsToOptions: function argsToOptions(args) {
      ...
      var options = nopt(knownOptionDefinitions,
                         ...
                         args,
                         0);
      options = self.processRemaining(options);
      return options;
    },

    processRemaining: function processRemaining(options) {
      var remain = options.argv.remain;
      var files = _.filter(remain, function (remain) {
        return remain.match(/^@\S+/);
      });

      files.forEach(function (file) {
        var features = self.getFeaturesFromFilePath(file.match(/^@(\S+)/));
    ...
    getFeaturesFromFilePath: function getFeaturesFromFilePath(filePath) {
      var features = fs.readFileSync(filePath[1], 'utf8');
