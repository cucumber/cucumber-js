  ...
  var self = {
    ...
    argsToOptions: function argsToOptions(args) {
      ...
      var options = nopt(knownOptionDefinitions,
                         ...
                         args,
                         0);
      options = self.processRemainingOptions(options);
      return options;
    },

    processRemainingOptions: function processRemainingOptions(options) {
      var remain = options.argv.remain;
      var files = _.filter(remain, function (remain) {
        return remain.match(ArgumentParser.RERUN_FILENAME_REGEXP);
      });

      files.forEach(function (file) {
        var filePath = path.resolve(file.replace(/^@/, ''));
        var features = self.getFeaturesFromFilePath(filePath);
        remain.push(features);
      });

      remain = _.filter(remain, function (remain) {
        return !_.include(remain, '@');
      });

      options.argv.remain = _.flatten(remain);

      return options;
    },

    getFeaturesFromFilePath: function getFeaturesFromFilePath(filePath) {
      var features = fs.readFileSync(filePath, 'utf8');
...
ArgumentParser.RERUN_FILENAME_REGEXP                     = /^@\S+/; //match any string starts with @
