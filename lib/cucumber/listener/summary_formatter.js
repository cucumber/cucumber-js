function SummaryFormatter(options) {
  ...
  self.logIssue = function logIssue(number, stepResult, message) {
    ...
    var scenarioLine = 'Scenario: ' + scenario.getName() + ' ' + colors.comment('# ' + scenarioLocation);
    lines.push(prefix + scenarioLine);

    var stepLine = 'Step: ' + step.getKeyword() + (step.getName() || '');
    if (step.hasUri()) {
      var stepLocation = path.relative(process.cwd(), step.getUri()) + ':' + step.getLine();
      stepLine += ' ' + colors.comment('# ' + stepLocation);
    }
    ...
    if (stepDefintion) {
      ...
      var stepDefinitionLine = 'Step Definition: ' + colors.comment(stepDefintionLocation);
