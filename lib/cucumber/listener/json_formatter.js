var JsonFormatter = function(options) {
  ...
  var blob = {"name": "",
              "id": ""} // Where we're going to stick the data we want to output as json
  ...
  var self = {
    log: function log(string) {
      ...
      if (options['logToConsole'])
        process.stdout.write(string); // This is where the results get blatted onto the console
    ...
    hear: function hear(event, callback) {
      ...
      if (self.hasHandlerForEvent(event)) {
        ...
        console.log('I GOT NOTHING FOR YA')
        callback();
    ...
    handleBeforeFeatureEvent: function handleBeforeFeatureEvent(event, callback) {
      blob["name"] = event.getPayloadItem('feature').getName();
    ...
    handleBeforeScenarioEvent: function handleBeforeScenarioEvent(event, callback) {
      ...
      var scenario = {"name": event.getPayloadItem('scenario').getName()};
      if (!("elements" in blob)) {
        blob["elements"] = [];
      }

      blob["elements"].push(scenario)
    ...
    handleStepResultEvent: function handleStepResult(event, callback) {
      ...
      if (!(blob["elements"][blob["elements"].length-1]["steps"])) {
        blob["elements"][blob["elements"].length-1]["steps"] = [];
      }
    ...
    handleUndefinedStepResult: function handleUndefinedStepResult(stepResult) {
      ...
      self.log(JsonFormatter.UNDEFINED_STEP_CHARACTER);
    },
    ...
    handleFailedStepResult: function handleFailedStepResult(stepResult) {
      ...
      self.log(JsonFormatter.FAILED_STEP_CHARACTER);
    },

    handleAfterFeaturesEvent: function handleAfterFeaturesEvent(event, callback) {
      self.logSummary();
      // Is this where we would dump the JSON?
      console.log(blob)
      callback();
    },
...
JsonFormatter.PASSED_STEP_CHARACTER     = '.';
JsonFormatter.SKIPPED_STEP_CHARACTER    = '-';
JsonFormatter.UNDEFINED_STEP_CHARACTER  = 'U';
JsonFormatter.PENDING_STEP_CHARACTER    = 'P';
JsonFormatter.FAILED_STEP_CHARACTER     = 'F';
