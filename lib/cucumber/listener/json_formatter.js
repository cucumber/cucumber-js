var JsonFormatter = function(io) {
  ...
  var gherkinJsonFormatter =  new GherkinJsonFormatter(io);
  ...
  self.formatStep = function formatStep(step) {
    ...
    if (step.hasDataTable()) {
      ...
      var tableProperties = [];
      for (rawRow in raw) {
        var row = {line: 'TODO', cells: raw[rawRow]};
  ...
  self.formatTags = function formatTags(tags, parent_tags) {
      var tagsProperties = [];
      for (tag in tags) {
  ...
  self.handleStepResultEvent = function handleStepResult(event, callback) {
    ...
    gherkinJsonFormatter.match({location: 'TODO'});
    callback();
  }
