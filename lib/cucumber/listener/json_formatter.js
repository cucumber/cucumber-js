...
function JsonFormatter(options) {
  ...
  self.handleBeforeFeatureEvent = function handleBeforeFeatureEvent(event, callback) {
    astFeature = event.getPayloadItem('feature');
    currentFeature = {
      description: astFeature.getDescription(),
      elements: [],
      id: astFeature.getName().replace(/ /g, '-').toLowerCase(),
      keyword: astFeature.getKeyword(),
      line: astFeature.getLine(),
      name: astFeature.getName(),
      tags: astFeature.getTags().map(formatTag),
      uri: astFeature.getUri()
    };
  ...
  self.handleStepResultEvent = function handleStepResultEvent(event, callback) {
    ...
    var astStep = astFeature.findStepByLine(step.getLine());
    ...
    var currentStep = {
      ...
      keyword: astStep ? astStep.keyword : '',
      name: step.getName(),
      result: {
        status: status
      }
