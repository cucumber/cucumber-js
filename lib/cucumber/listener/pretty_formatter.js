function PrettyFormatter(options) {
  ...
  self.handleBeforeScenarioEvent = function handleBeforeScenarioEvent(event, callback) {
    ...
    var uri = color.format('comment', '# ' + path.relative(process.cwd(), scenario.getUri()) + ':' + scenario.getLine());
    source += uri + '\n';
  ...
  self.logStepResult = function logStepResult(step, stepResult) {
    ...
    if (step.hasUri()) {
      source += color.format('comment', '# ' + path.relative(process.cwd(), step.getUri()) + ':' + step.getLine());
    }
