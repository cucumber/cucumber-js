var PrettyFormatter = function(options) {
  ...
  var self = {
    ...
    handleBeforeFeaturesEvent: function handleBeforeFeaturesEvent(event, callback) {
      startTime = new Date();
      callback();
    },
    ...
    logScenario: function logScenarios(scenario) {
        self.log("  "+scenario.getKeyword()+": "+scenario.getName()+"\n");
    },

    logStepResult: function logStepResult(stepResult, step) {
        var color;
        ...
        self.log(ansiColor("    "+step.getKeyword()+step.getName(), color)+"\n");
    },
    ...
    logSummary: function logSummary() {
      ...
      self.log(((endTime-startTime)/1000.0)+"s\n");
      if (self.witnessedAnyUndefinedStep() && options["logSnippets"])
        self.logUndefinedStepSnippets();

      if (failedStepResults.length() > 0) {
        self.log("\n");
        failedStepResults.syncForEach(function(step, i, steps) {
          self.logFailedStepResult(step);
          self.log("\n");
        });
      }
    },
