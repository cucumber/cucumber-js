function SimpleAstListener(options) {
  ...
  var currentAstStep, currentFeature, currentStep;
  ...
  var self = {
    ...
    hearBeforeStep: function hearBeforeStep(step, callback) {
      currentStep = step;
      currentAstStep = currentFeature.findStepByLine(step.getLine());
      callback();
    },

    hearStepResult: function hearStepResult(stepResult, callback) {
      log(currentAstStep.keyword + (currentStep.getName() || ''), 2);
