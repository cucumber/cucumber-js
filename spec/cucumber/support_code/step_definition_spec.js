...
describe('Cucumber.SupportCode.StepDefinition', function () {
    ...
    beforeEach(function () {
        pattern = createSpyWithStubs('pattern', {
            test: null
        });
    ...
    describe('getPatternRegexp()', function () {
        ...
        describe('when the pattern is a String', function () {
            var regexp,
                quotedDollarParameterSubstitutedPattern,
                safeString,
                regexpString;
            ...
            beforeEach(function () {
                ...
                quotedDollarParameterSubstitutedPattern = createSpyWithStubs('quoted dollar param substituted pattern', {
            ...
            it('escapes unsafe regexp characters from the string', function () {
                ...
                expect(pattern.replace)
                    .toHaveBeenCalledWith(Cucumber.SupportCode.StepDefinition.UNSAFE_STRING_CHARACTERS_REGEXP, Cucumber.SupportCode.StepDefinition.PREVIOUS_REGEXP_MATCH);
            });

            it('replaces quoted dollar-prefixed parameters with the regexp equivalent', function () {
                stepDefinition.getPatternRegexp();
                expect(safeString.replace)
                    .toHaveBeenCalledWith(Cucumber.SupportCode.StepDefinition.QUOTED_DOLLAR_PARAMETER_REGEXP, Cucumber.SupportCode.StepDefinition.QUOTED_DOLLAR_PARAMETER_SUBSTITUTION);
            });

            it('replaces other dollar-prefixed parameter with the regexp equivalent', function () {
                stepDefinition.getPatternRegexp();
                expect(quotedDollarParameterSubstitutedPattern.replace)
                    .toHaveBeenCalledWith(Cucumber.SupportCode.StepDefinition.DOLLAR_PARAMETER_REGEXP, Cucumber.SupportCode.StepDefinition.DOLLAR_PARAMETER_SUBSTITUTION);
            });
    ...
    describe('matchesStepName()', function () {
        ...
        beforeEach(function () {
            ...
            patternRegexp = createSpyWithStubs('pattern regexp', {
                test: matchResult
            });
        ...
        describe('callback passed to the step definition code', function () {
            ...
            describe('when called without an error', function () {
                ...
                it('creates a successful step result', function () {
                    expect(Cucumber.Runtime.SuccessfulStepResult).toHaveBeenCalledWith({
                        step: step,
                        duration: 1e6
                    });
                });
            ...
            describe('pending()', function () {
                ...
                it('creates a pending step result', function () {
                    ...
                    expect(Cucumber.Runtime.PendingStepResult).toHaveBeenCalledWith({
                        step: step,
            ...
            describe('fail()', function () {
                ...
                it('creates a failing step result', function () {
                    ...
                    expect(Cucumber.Runtime.FailedStepResult).toHaveBeenCalledWith({
                        step: step,
    ...
    describe('buildInvocationParameters()', function () {
        var patternRegexp,
            step,
            stepName,
            stepAttachmentContents,
            matches,
            callback;

        beforeEach(function () {
            stepName = createSpy('step name to match');
            matches = createSpyWithStubs('matches', {
                shift: null,
                push: null
            });
            patternRegexp = createSpyWithStubs('pattern regexp', {
            ...
            step = createSpyWithStubs('step', {
                hasAttachment: null,
                getName: stepName,
                getAttachmentContents: stepAttachmentContents
            });
            callback = createSpy('callback');
            spyOn(stepDefinition, 'getPatternRegexp').andReturn(patternRegexp);
    ...
    describe('buildExceptionHandlerToCodeCallback()', function () {
        ...
        beforeEach(function () {
            codeCallback = createSpyWithStubs('code callback', {
                fail: null
            });
