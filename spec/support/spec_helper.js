beforeEach(function () {
    ...
    this.addMatchers({
        ...
        toHaveBeenCalledNTimes: function (callCount) {
            return callCount === this.actual.callCount;
        },
        ...
        toHaveBeenCalledWithStringMatching: function (pattern) {
            for (var i = 0; i < this.actual.callCount; i++) {
                ...
                if ((pattern.test && pattern.test(parameter)) || (typeof (pattern) === 'string' && parameter.indexOf(pattern) >= 0)) {
                    return true;
                }
