...
export class MultiProcessCoordinatorAdapter {
  ...
  private parseWorkerMessage(
    worker: IWorker,
    message: ICoordinatorReport
  ): void {
    if (message.ready) {
  ...
  private awakenWorkers(triggeringWorker: IWorker): void {
  ...
  private startWorker(id: string, total: number): void {
    const workerProcess = fork(runWorkerPath, [], {
      cwd: this.cwd,
      env: {
        ...process.env, // TODO use env from IRunEnvironment
  ...
  private onWorkerProcessClose(exitCode: number): void {
  ...
  private nextPicklePlacement(): IPicklePlacement {
  ...
  private placementAt(index: number): IPicklePlacement {
    return {
      index,
  ...
  private giveWork(worker: IWorker, force: boolean = false): void {
